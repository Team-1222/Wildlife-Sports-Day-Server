...
- `develop`, `release/**`를 포함한 `main` 이외 브랜치에 push하면 app/migrator 개발 이미지를 비공개 GHCR에 게시합니다. PR과 수동 개발 CI 실행은 빌드·테스트만 수행합니다.
- `main` 반영 시 공통 CI를 통과한 app/migrator 후보 이미지를 빌드하고, GitHub Actions 안에서 PostgreSQL, EF migrator, app, Caddy를 임시 실행해 배포를 리허설합니다.
- 리허설을 통과한 동일 immutable image digest를 운영 태그로 승격하고, Tailscale SSH로 로컬 WSL 운영 환경에 배포합니다.
